...
void* exec_th1_goldbach_singolo(void* arg) {
    SharedData* shared = (SharedData*)arg;
    unsigned int part = shared->result; // Inizializza part con il valore corrente
    
    while (part <= shared->final && goldbach_singolo(part)) part += 4;
    pthread_mutex_lock(&shared->mutex);
    if (part > shared->result)
        shared->result = part;
    pthread_mutex_unlock(&shared->mutex);
    return NULL;
}

void* exec_th2_goldbach_singolo(void* arg) {
    SharedData* shared = (SharedData*)arg;
    unsigned int part = shared->result+2; // Inizializza part con il valore corrente
...
unsigned int golbach_sequenza(unsigned int n) {
    ...
    SharedData shared = {4, n, PTHREAD_MUTEX_INITIALIZER}; // Inizializza shared con 4
    
    pthread_t t1, t2;
    pthread_create(&t1, NULL, exec_th1_goldbach_singolo, &shared);
    pthread_create(&t2, NULL, exec_th2_goldbach_singolo, &shared);
    
    pthread_join(t1, NULL);
    pthread_join(t2, NULL);
    
    pthread_mutex_destroy(&shared.mutex);
    return (shared.result <  n) ? shared.result : 0;
}